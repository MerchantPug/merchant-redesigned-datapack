...
execute if score on_modify ioi-pi matches 1 if entity @s[tag = merchant.gravel_brush] if data storage ioi-pi:output {item:{id:"minecraft:knowledge_book"}} if score #temp modelData matches 127156 run playsound minecraft:entity.villager.work_cartographer player @a ~ ~ ~ 1.0 0.75
