...
tellraw @a {"score":{"name":"#temp","objective":"merchantVillagerId"}}
...
execute as @e[type = minecraft:marker, tag = merchant.villager_paired] if score @s id = #currentConversion id at @s run schedule function merchantpug:villager/transfer_scores/transfer_scores_zombie 1t
