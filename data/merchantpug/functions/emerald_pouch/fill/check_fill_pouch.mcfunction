...
execute if score on_modify ioi-pi matches 1 if entity @s[tag = merchant.fill_pouch] if data storage ioi-pi:output {item:{id:"minecraft:knowledge_book"}} if score #temp isEmeraldPouch matches 1 run function merchantpug:emerald_pouch/fill/fill_pouch
